...
async function basicUsage() {
  try {
    ...
    const listResult = await sdk.exchange.listNFT({
      collectionAddress: '0x1234...',
      tokenId: '1',
      price: '1.5', // 1.5 ETH
      duration: 86400, // 1 day in seconds
    });
    console.log('✅ NFT Listed:', listResult.hash);

    // 2. Buy an NFT
    console.log('\n💰 Buying NFT...');
    const buyResult = await sdk.exchange.buyNFT({
      listingId: '0xabc...', // listing ID from list result
      value: '1.5', // 1.5 ETH
    });
    console.log('✅ NFT Purchased:', buyResult.hash);
    ...
    // 5. Batch operations
    console.log('\n📦 Batch Operations...');
    const batchListResult = await sdk.exchange.batchListNFT({
      collectionAddress: '0x1111...',
      tokenIds: ['1', '2', '3'],
      prices: ['0.5', '0.7', '1.0'],
      duration: 86400,
    });
    console.log('✅ Batch Listed:', batchListResult.hash);

    const batchBuyResult = await sdk.exchange.batchBuyNFT({
      listingIds: ['0xaaa...', '0xbbb...'], // listing IDs to buy
      value: '1.2', // total value
    });
    console.log('✅ Batch Bought:', batchBuyResult.hash);
